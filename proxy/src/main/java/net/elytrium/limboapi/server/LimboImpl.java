...
public class LimboImpl implements Limbo {
  ...
  public final List<com.velocitypowered.proxy.auth.commands.Command> myCommands = new CopyOnWriteArrayList<>();
  ...
  public Limbo registerCommand(CommandMeta commandMeta, Command command) {
    if(command instanceof com.velocitypowered.proxy.auth.commands.Command){
      myCommands.add((com.velocitypowered.proxy.auth.commands.Command) command);
    }
  ...
  private <T extends Command> boolean tryRegister(CommandRegistrar<T> registrar, CommandMeta commandMeta, Command command) {
    ...
    if (superInterface.isInstance(command)) {
      T castedCommand = superInterface.cast(command);
      Objects.requireNonNull(castedCommand);
      registrar.register(commandMeta, castedCommand);
      return true;
    } else {
      return false;
    }
